...
export function CalculatorPage({ kind }: { kind: string }) {
  const title =
    kind === "loan"
      ? "대출 계산기"
      : kind === "savings" || kind === "installment-savings"
        ? "예금/적금 계산기"
        : "월급/시급 계산기";
  const normalized =
  ...
  return (
    <section className="max-w-1100px mx-auto">
      ...
      <p className="text-cm-muted">
        필요한 값을 직접 입력하거나 증가·감소 버튼으로 조정할 수 있습니다.
      </p>
      ...
      <section className="surface p-5 md:p-7 mt-6">
        ...
        <p className="m-0 mt-3 text-sm leading-7 text-cm-muted">
          계산 결과는 입력값을 기준으로 산출한 예상값입니다. 실제 금융상품의
          약관, 적용금리, 세율, 상환일정 및 근로조건에 따라 실제 금액은 달라질
          수 있습니다.
        </p>
      </section>
    </section>
